...
openssl req -x509 -newkey rsa:4096 -days 365 -nodes -keyout ca-key.pem -out ca-cert.pem -subj "/C=US/ST=CA/L=LosAngeles/O=ISI/OU=Research/CN=MetisFL-AI/emailAddress=metisfl@metisfl.ai"
...
openssl req -newkey rsa:4096 -days 3600 -nodes -keyout server-key.pem -out server-req.pem -subj "/C=US/ST=CA/L=LosAngeles/O=ISI/OU=Research/CN=localhost/emailAddress=metisfl@metisfl.ai"
...
openssl req -newkey rsa:4096 -days 3600 -nodes -keyout client-key.pem -out client-req.pem -subj "/C=US/ST=CA/L=LosAngeles/O=ISI/OU=Research/CN=localhost/emailAddress=metisfl@metisfl.ai"
