...
class HousingMLP(torch.nn.Module):
    def __init__(self, params_per_layer=10, hidden_layers_num=1, learning_rate=0.0, data_type="float32"):
        ...
        layers.append(torch.nn.Linear(
            13, self.params_per_layer, dtype=self.data_type))
        layers.append(torch.nn.ReLU())
        for i in range(self.hidden_layers_num):
            layers.append(torch.nn.Linear(
                self.params_per_layer, self.params_per_layer, dtype=self.data_type))
            layers.append(torch.nn.ReLU())
        layers.append(torch.nn.Linear(self.params_per_layer, 1))
    ...
    def fit(self, dataset, epochs, *args, **kwargs) -> Dict:
        ...
        criterion = MSELoss()
        optimizer = SGD(self.parameters(), lr=1e-15, momentum=0.9)
        # enumerate epochs
